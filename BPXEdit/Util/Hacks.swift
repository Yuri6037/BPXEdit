...
//isNumber does not actually check wither the character is numeric, it allows for more character which should be forbidden.
extension Character {
    var isNumeric: Bool { asciiValue ?? 0 >= 0x30 && asciiValue ?? 0 <= 0x39 }
}
